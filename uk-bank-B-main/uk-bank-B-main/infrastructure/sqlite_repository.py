...
class SQLiteAccountRepository(AccountRepository):
    ...
    def get_by_id(self, account_id: AccountNumber) -> Optional[Account]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT balance, currency, is_active FROM accounts WHERE sort_code = ? AND account_number = ?",
            ...
            balance_str, currency_str, is_active = row
            balance = Money(Decimal(balance_str), Currency(currency_str))

            return Account(id=account_id, balance=balance, is_active=bool(is_active))
    ...
    def save(self, account: Account) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                DO UPDATE SET balance=excluded.balance, is_active=excluded.is_active
            """, (
